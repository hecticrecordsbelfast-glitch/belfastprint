/* ------------------------------------------------------------------
   ...
   t-shirt order. Price is looked up from the saved design record
   (£20 for one design, £25 for two) rather than trusted from the
   browser, so it can't be tampered with.
...
const PRICE_ONE_DESIGN = 20;
const PRICE_TWO_DESIGNS = 25;
...
exports.handler = async (event) => {
  ...
  const price = design.numDesigns >= 2 ? PRICE_TWO_DESIGNS : PRICE_ONE_DESIGN;
  const designLabel = design.numDesigns >= 2 ? "2 designs" : "1 design";
  ...
  const payload = {
    ...
    order: {
      ...
      line_items: [
        {
          name: `Custom T-Shirt — ${design.size}, ${design.color === "black" ? "Black" : "White"}, ${designLabel}`,
          quantity: "1",
