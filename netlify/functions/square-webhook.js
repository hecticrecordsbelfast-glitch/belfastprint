...
async function sendOrderEmail(orderRecord, design) {
  ...
  const attachments = [];
  const preview = dataUrlToAttachment(design.previewImage, "shirt-preview.png");
  if (preview) attachments.push(preview);
  design.originals.forEach((o, i) => {
    const att = dataUrlToAttachment(o.dataUrl, o.filename || `design-${i + 1}.png`);
...
Size: ${design.size}
Colour: ${design.color === "black" ? "Black" : "White"}
Designs: ${design.numDesigns}
...
The shirt preview (with design placement) and original artwork file(s) are attached.
