...
/* ---------- Colour & size (loaded from /api/settings) ---------- */

function applyShirtColor(hex) {
  document.querySelectorAll(".shirtBody").forEach((el) => el.setAttribute("fill", hex));
}

function renderColorSwatches() {
  const wrap = document.getElementById("colorSwatches");
  wrap.innerHTML = COLORS.map((c) => `
    <button type="button" class="swatch ${c.id === state.color ? "is-selected" : ""}" data-color-id="${c.id}" style="background:${c.hex};" aria-label="${c.name}"></button>
  `).join("");
}
document.getElementById("colorSwatches").addEventListener("click", (e) => {
  const btn = e.target.closest("[data-color-id]");
  if (!btn) return;
  state.color = btn.getAttribute("data-color-id");
  const colorEntry = COLORS.find((c) => c.id === state.color);
  renderColorSwatches();
  if (colorEntry) applyShirtColor(colorEntry.hex);
});

function renderSizeButtons() {
  const wrap = document.getElementById("sizeRow");
  const ordered = [...SIZES].sort((a, b) => SIZE_ORDER.indexOf(a.size) - SIZE_ORDER.indexOf(b.size));
  wrap.innerHTML = ordered.map((s) => {
    const soldOut = s.stock <= 0;
    return `<button type="button" class="size-btn ${s.size === state.size ? "is-selected" : ""} ${soldOut ? "is-soldout" : ""}" data-size="${s.size}" ${soldOut ? "disabled" : ""}>${s.size}</button>`;
  }).join("");
  updatePayState();
}
...
async function loadSettings() {
  ...
  const firstInStock = [...SIZES].sort((a, b) => SIZE_ORDER.indexOf(a.size) - SIZE_ORDER.indexOf(b.size)).find((s) => s.stock > 0);
  state.size = firstInStock ? firstInStock.size : (SIZES[0] ? SIZES[0].size : null);
...
function updatePayState() {
  ...
  const currentSizeEntry = SIZES.find((s) => s.size === state.size);
  const sizeAvailable = !!currentSizeEntry && currentSizeEntry.stock > 0;
  const ready = frontReady && (!needsBoth || backReady) && sizeAvailable;

  payBtn.disabled = !ready;
  if (!sizeAvailable) {
    payBtn.textContent = state.size ? `Size ${state.size} is sold out` : "All sizes sold out";
  } else if (!frontReady) {
...
payBtn.addEventListener("click", async () => {
  ...
  const currentSizeEntry = SIZES.find((s) => s.size === state.size);
  const sizeAvailable = !!currentSizeEntry && currentSizeEntry.stock > 0;
  if (!frontReady || (needsBoth && !backReady) || !sizeAvailable) return;
